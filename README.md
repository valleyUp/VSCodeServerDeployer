...
- When `-e` is used, the script will emit both a core package (server/cli) and an extensions-only package.

The script produces `vscode-server-core-<commit_id>-<arch>.tar.xz` (server/cli only). Extract this archive to the home directory of the target machine and connect using VS Code Remote SSH. When `-e` is used, it also produces `vscode-server-extensions-<commit_id>-<arch>.tar.xz` containing only `.vscode-server/extensions` for extension-only updates. Install extensions from VS Code online or sync if you skip `-e`.
...
## 概述
该仓库提供了一个 Bash 脚本，用于根据指定的 commit 下载 VS Code Server 与 CLI，并打包成离线安装包。扩展预装调用官方的 server 端 CLI，与 VS Code 的安装行为保持一致。仓库内包含的 GitHub Action 工作流会自动下载并发布 x64 与 arm64 的 core 包（仅 server/cli），并生成 extensions 包（仅扩展），便于分别升级；压缩使用 `xz -9e`。
...
脚本执行后会生成 `vscode-server-core-<commit_id>-<arch>.tar.xz`，将其解压到目标服务器的用户主目录即可通过 VS Code Remote SSH 连接；若指定了 `-e`，还会生成 `vscode-server-extensions-<commit_id>-<arch>.tar.xz`，仅包含 `.vscode-server/extensions`，适合只同步扩展而不动现有 server。
